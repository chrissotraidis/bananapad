...
| Medium | G3/G6 | Full RSP/audio behavior and route-change matrix is not complete; Issue #1 reports periodic popping on an iPhone 15 Pro Max. | The output/input-rate queue-duration mismatch is corrected and diagnostics now measure starvation, decimation, submission gaps, boundaries, and errors. Reproduce on physical iPhone and correlate an audible pop with the new two-second telemetry before changing the resampler. |
