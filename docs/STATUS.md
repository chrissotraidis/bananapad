...
- **Active goal:** roll the accepted Apple-shell work into private `main` and produce the first exact ROM-free unsigned IPA candidate. Remaining physical controller/audio/soak coverage continues independently as preview-quality work.
- **Current step:** Product source `4d223ee7f4bd8ed536b2238616c2ac1a23ae3584f66a5869197b0afc349a0d8a` preserves the PaperPad-derived touch/menu behavior with BananaPad labels, safe ROM management, controller/native-UI suppression, and the SDL-level Xbox A duplication repair. The release path now adds BananaPad-specific notices, complete pinned license collection, reproducible source-path mapping, deterministic packaging, checksum output, and expanded IPA audits. Signed hardware executable `0cf47f413d544a59d6fd7b96b4838a4325b03044bd40f217226b47711f1b5abd` remains installed in place with the user's ROM/save/preferences preserved; the unsigned release executable is rebuilt separately and does not alter that device container.
- **Rights state:** the private `main` push and exact local IPA-candidate build/audit are authorized. Repository visibility, a public tag/release, and IPA upload remain separate publication actions.
...
| G12–G13 | G12 started; G13 local candidate authorized | Signed iPad build/install/launch and visible DK64 rendering pass with an artifact-bound preflight receipt. G12 still requires the worksheet's controller reconnect, audible audio routes/interruptions, memory pressure, and sustained-operation checks. G13 now permits private-main source rollup and exact local IPA creation/audit, while public visibility/tag/upload remain unapproved. |
...
Do not replay DK64 progression without a named BananaPad regression. The mobile source, Simulator build, signed universal iPhoneOS build, in-place physical-iPad install/launch, visible hardware rendering, BananaPad-branded PaperPad-derived touch/three-dot UI, ROM safety, native-UI suppression/neutral rearm, packaging, README, and reversible upstream lane are green. The physical preflight binds the exact signed bundle, source, and resolved device into a private pending receipt and worksheet. The next step is the remaining hands-on G12 worksheet: physical multi-finger chords, real-controller ownership, audible audio routes/interruptions, sustained thermal/memory behavior, and icon appearances. Simulator gameplay is not a substitute.
