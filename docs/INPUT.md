...
The Apple shell keeps PaperPad's pinned N64 overlay presentation while adapting its input behavior for DK64. It exposes the analog stick, D-pad, A, B, Z, L, R, Start, and all four C directions. Phone and tablet defaults are independent, safe-area aware, locally persistent, editable, and resettable. Each `UITouch` owns its own role, so holding Z while pressing A, B, or a C direction does not serialize or release the first finger. Short taps are latched long enough for the emulated input poll to observe them. Holding Z continuously for 0.75 seconds engages the visible Z lock; when locked, holding Z for 0.35 seconds releases it. The shorter release gesture responds to Issue #2 feedback while the longer engage threshold reduces accidental locking during ordinary Z holds. The `Hold Z to Lock` setting is on by default and can be disabled from the three-dot menu's Settings sheet.
