...
| 34 | Exact public candidate | Met | Root GPL text, copied/adapted-file provenance, notices, deterministic packaging, expanded auditing, public-source decision, exact IPA decision, and official release identity are complete. Hosted-byte verification is the final publication check; broader physical iPad/iPhone acceptance remains separate preview work. |
...
1. Download the hosted IPA and checksum, compare them with the exact local artifacts, and verify the hosted checksum before announcing completion.
2. Continue the remaining Apple-shell matrix independently: controller reconnect, audio interruption/routes, memory pressure, and sustained iPad/iPhone operation. Preview status must not imply those unobserved checks passed.
