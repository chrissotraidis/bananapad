...
rg -q 'kZLockHoldSeconds = 0\.75' "$ui" || die "Z lock does not use the 0.75-second engage contract"
rg -q 'kZUnlockHoldSeconds = 0\.35' "$ui" || die "Z unlock does not use the faster 0.35-second release contract"
rg -q '_zLocked \? kZUnlockHoldSeconds : kZLockHoldSeconds' "$ui" || die "Z lock does not select engage/release timing from its current state"
