...
- **G5. macOS first-play loop.** Import/verify ROM → title/intro → create or select an Adventure file → complete the opening Training Grounds route → reach DK Isles → collect the first Golden Banana and enter Jungle Japes → complete one documented in-level Golden Banana objective → allow progress to persist → clean exit/relaunch/load. This is the minimum BananaPad feasibility proof. (PRD D4)
- **G6. Complete macOS progression and special-code coverage.** A fresh-save golden path reaches every required world through Hideout Helm, defeats the required bosses and final K. Rool sequence, obtains the Nintendo and Rareware coins through the Donkey Kong arcade and Jetpac paths, reaches credits, and reloads the completed save. All required Kong, vendor, move, weapon, instrument, camera, transformation, minecart, race, bonus, critter, boss, menu, arcade, and Jetpac paths have evidence. (PRD D5–D6)
- **G7. Correctness, enhancements, and stability.** Baseline timing is documented; original-framing/original-cadence behavior is stable; upstream high-framerate, widescreen, analog-camera, draw-distance, and related enhancements are enabled only where their mobile matrix passes; rendering, audio routes, input response, save compatibility, repeated transitions, memory behavior, and at least one 90-minute soak are green. (PRD D7, D10)
- **G8. iPad Simulator core accepted.** With every other Simulator shut down, the iPad Simulator builds and completes the first-play loop with the same AOT core, static patches, overlay model, Metal renderer, save format, and no-dynamic-code profile. Simulator performance is diagnostic only.
- **G9. iPhone Simulator core accepted.** After shutting down the iPad Simulator, the iPhone Simulator builds and completes the first-play loop with the same core. Compact safe areas, landscape orientations, memory pressure, touch reachability, and native UI behavior are evidenced.
- **G10. SunPad/PaperPad Apple shell complete.** SunPad's pinned three-dot menu, touch-layout, settings, diagnostics, controller, lifecycle, and evidence machinery are adapted to DK64; PaperPad's pinned N64/RT64 ROM, input, Metal, AOT-only, package-safety, and shutdown mechanisms are reused where appropriate. DK64-specific multi-touch chords, camera controls, ROM management, saves, controller handoff, audio interruptions/routes, loading states, and an original BananaPad app icon work on both form factors. (PRD D8–D9)
...
G2 is the initial upstream comparison and synchronization gate. G4 and G5 are the first hard mobile-architecture and playability gates. G6 and G11 are the technical release bar. G12 and G13 are mandatory for a public binary. There is no fallback to a title-screen demo, Jungle-Japes-only build, controller-only mobile build, dynamic-code sideload build, untested enhancement build, or “ROM-free therefore cleared” release.
...
- **RSP is part of the product.** `n_aspMain` generation, registration, queue behavior, music, ambience, voices, UI sounds, weapons, instruments, arcade/Jetpac audio, transitions, and credits require evidence. No silent HLE or audio-disable fallback.
...
5. **Check SunPad.** Read the pinned `SunPadGameOverlay`, `SunPadGameViewController`, shared settings/input/controller/diagnostics code, tests, app-icon provenance pattern, and three-dot menu. Port the interaction architecture, not Sunshine-specific button semantics or Dolphin runtime assumptions.
...
- **Per session:** run host regressions plus a boot/end-to-end smoke on the highest known-good target. End the journal with the exact known-good command, revision, artifact, ROM identity, save state, and next lowest goal.
- **Per candidate:** rerun the full applicable matrix against the exact artifact. Do not combine evidence from older builds.
- **Honesty rule:** configured or source-inspected behavior is not acceptance. If it was not run and observed, it is not done. A full-play claim and a 101% claim are separate and must never be inferred from a later-game fixture.
...
- **PaperPad is the N64-on-Apple substrate reference.** Pin `ref/paperpad` to `74b6e45830a06c7f274c5ac1ddd7c625bc13a557`. Reuse its AOT-only build shape, `N64MODERN_NO_DYNAMIC_CODE` route, RT64/Metal fixes, ROM manager, N64 touch/input bridge, controller ownership, lifecycle, diagnostics, dependency locks, clean-clone scripts, and package audits where applicable.
- **SunPad is the menu/touch product reference.** Pin `ref/sunpad` to `e43f0ea6b797e5110787171957c9dc3c6213269c`. Use its three-dot menu, loading-state honesty, layout editing, device-specific defaults, normalized input, controller handoff, Game Data & Saves organization, report flow, settings discipline, app-icon provenance, and tests. Translate controls to DK64; do not copy GameCube runtime or Sunshine assumptions.
...
- **Scripts:** provide deterministic equivalents of `check-prerequisites.sh`, `clone-sources.sh`, `verify-sources.sh`, `check-upstream.sh`, `stage-upstream-update.sh`, `test-upstream-update.sh`, `promote-upstream-update.sh`, `rollback-upstream-update.sh`, `prepare-rom.sh`, `build-host-tools.sh`, `generate-game.sh`, `generate-patches.sh`, `build-upstream-macos-baseline.sh`, `build-macos-app.sh`, `build-ios-simulator.sh`, `build-ios-device.sh`, `run-smoke.sh`, `capture-crashes.sh`, `check-no-dynamic-code.sh`, `check-repo-safety.sh`, `audit-ios-package.sh`, and `package-unsigned-ipa.sh`.
...
2. Run the host regression suite and highest known-good smoke test, or record exactly why one cannot run.
